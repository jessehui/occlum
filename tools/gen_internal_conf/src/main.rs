...
fn gen_app_config(
    ...
    *app_config.pointer_mut("/app/1/entry_points").unwrap() = entry_points;
        ...
        .find(|m| m.target == String::from("/") && m.type_ == String::from("unionfs"))
    {
        ...
        *app_config.pointer_mut("/app/1/mount/0").unwrap() = serde_json::to_value(root_mc).unwrap();
    }
    ...
    *app_config
    ...
    if let Some(host_mc) = mount_config
        .iter_mut()
        .find(|m| m.type_ == String::from("hostfs"))
    {
        debug!("User provides host mount config:\n{:?}", host_mc);
        // Update app root mount
        *app_config.pointer_mut("/app/1/mount/1").unwrap() = serde_json::to_value(host_mc).unwrap();
    }
