...
pub const CHUNK_DEFAULT_SIZE: usize = 32 * 1024 * 1024; // 32MB
...
impl Chunk {
    ...
    pub fn new_single_vma_chunk(vm_range: VMRange, options: &VMMapOptions) -> Self {
        ...
        let vm_area = VMArea::new(vm_range.clone(), *options.perms(), writeback_file);
    ...
    // For chunks with ChunkType::MultiVMA, there could be unmapped range when the whole chunk is freed.
    pub fn clean_multi_vmas(&self) {
        match self.internal() {
            ChunkType::SingleVMA(vma) => unreachable!(),
...
impl ChunkInternal {
    ...
    pub fn remove_current_process(&mut self) {
        let current_pid = current!().process().pid();
        debug_assert!(self.process_set.remove(&current_pid) == true);
    }
