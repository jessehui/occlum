...
impl InternalVMManager {
    ...
    fn free_fast_default_chunks(&mut self) {
        let mut default_chunks = Vec::new();
        std::mem::swap(&mut self.fast_default_chunks, &mut default_chunks);

        default_chunks.into_iter().for_each(|chunk| {
            self.free_manager
