...
impl<'a, 'b> ProcessVMBuilder<'a, 'b> {
    ...
    pub fn build(self) -> Result<ProcessVM> {
        ...
        elf_layouts
            ...
            .map(|(elf_layout, elf_file)| {
                let vm_option = VMMapOptionsBuilder::default()
                    ...
                    // We only load loadable segments, thus just commit the memory when allocating.
        ...
        let vm_option = VMMapOptionsBuilder::default()
            ...
            .perms(VMPerms::READ | VMPerms::WRITE)
            // Since the process stack memory will definitely be used later and the stack memory doesn't take too much space, we just commit the memory when creating the process VM.
            .page_policy(PagePolicy::CommitNow)
            .build()
            .map_err(|e| {
                &self.handle_error_when_init(&chunks);
...
impl ProcessVM {
    ...
    pub fn mmap(
        ...
        let page_policy = PagePolicy::CommitOnDemand;
