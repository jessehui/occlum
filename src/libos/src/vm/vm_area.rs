...
// pub fn vma_mut(&mut self) -> &mut VMArea {
//     &mut self.vma
// }

// fn commit_current_vma(&mut self) -> Result<()> {
//     self.pages.commit_current_vma(true)?; // also need to update the global tracker
//     Ok(())
// }

// Same as L2 cache size
// const COMMIT_MEMORY_SIZE_THRESHOLD: usize = 4096 * 1024;

// pub fn need_write_back(&self) -> bool {
//     if let Some(file) = &self.file_backed {
//         file.need_write_back()
//     } else {
//         false
//     }
// }

// pub fn once_commit_size(&self, pf_page_id: usize) -> usize {
//     // Three cases for commit size:
//     // 1. from pf_page_start_addr to the committed page start address
//     // 2. from pf_page_start_addr to the end of the vma
//     // 3. from pf_page_start_addr to the commit memory size threashold

//     // Get commited page id after the page fault
//     let committed_page_id = self.pages.inner[pf_page_id..].first_one();
//     if let Some(committed_page_id) = committed_page_id {
//         if committed_page_id - pf_page_id > (COMMIT_MEMORY_SIZE_THRESHOLD / PAGE_SIZE) {
//             //
//             COMMIT_MEMORY_SIZE_THRESHOLD
//         } else {
//             (committed_page_id - pf_page_id) * PAGE_SIZE
//         }
//     } else {
//         // all pages after page-fault page are not committed
//         let pf_page_start_addr = self.range().start() + PAGE_SIZE * pf_page_id;
//         if self.range().end() - pf_page_start_addr > COMMIT_MEMORY_SIZE_THRESHOLD {
//             COMMIT_MEMORY_SIZE_THRESHOLD
//         } else {
//             self.range().end() - pf_page_start_addr
//         }
//     }
// }

// pub fn commit_pages(&mut self, pf_addr: usize) -> (usize, usize) {
//     let vma_start_addr = self.range().start();
//     let page_id = (pf_addr - vma_start_addr) / PAGE_SIZE;
//     let pf_page_start_addr = vma_start_addr + page_id * PAGE_SIZE;
//     debug_assert!(align_down(pf_addr as usize, PAGE_SIZE) == pf_page_start_addr);

//     let commit_size = self.once_commit_size(page_id);
//     self.pages.inner[page_id..page_id + commit_size / PAGE_SIZE]
//         .iter_mut()
//         .for_each(|mut bit| *bit = true);
//     (pf_page_start_addr, commit_size)
// }

// // Advice for commit memory. Return (commit addr, commit size)
// pub fn page_fault_advice(&self, pf_addr:usize) -> (usize, usize) {
//     let vma_start_addr = self.range().start();
//     let page_id = (pf_addr - vma_start_addr)/ PAGE_SIZE;
//     let pf_page_start_addr = vma_start_addr + page_id * PAGE_SIZE;
//     debug_assert!(align_down(pf_addr as usize, PAGE_SIZE) == pf_page_start_addr);
