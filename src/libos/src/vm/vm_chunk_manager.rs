...
impl ChunkManager {
    pub fn from(addr: usize, size: usize) -> Result<Self> {
        ...
        let vmas = {
            ...
            let start_sentry = {
                ...
                VMAObj::new_vma_obj(VMArea::new(range, perms, None))
            };
            let end_sentry = {
    ...
    // The whole range of ChunkManager will be freed when process exit. There is no need
    // to track the vmas anymore. Just make sure all vmas are cleaned or flushed.
    pub fn clean_all_vmas(&mut self) {
        self.vmas
            .iter()
            .filter(|&vma_obj| vma_obj.vma().size() != 0)
            .for_each(|vma_obj| {
                let vma = &vma_obj.vma();
                warn!("clean vma = {:?}", vma);
                // File-backed VMA needs to be flushed upon free
    ...
    pub fn parse_mremap_options(&mut self, options: &VMRemapOptions) -> Result<VMRemapResult> {
        ...
        let containing_vma = {
                ...
                && vmas_cursor.get().unwrap().vma().start() <= old_range.end()
            {
                ...
                if vma.is_superset_of(&old_range) {
                    break;
                } else {
                    vmas_cursor.move_next();
                    continue;
            ...
            if intersection_vma.range() == containing_vma.range() {
                ...
                match remain_vmas.len() {
                    2 => {
                        ...
                        // New VMA
                        let new_vma =
                            VMArea::inherits_file_from(&containing_vma, protect_range, new_perms);
                        VMPerms::apply_perms(&new_vma, new_vma.perms());
    ...
    pub fn find_mmap_region(&self, addr: usize) -> Result<VMRange> {
        ...
        if vma.is_null() || !vma.get().unwrap().vma().contains(addr) {
            return_errno!(ESRCH, "no mmap regions that contains the address");
        }
        return Ok(vma.get().unwrap().vma().range().clone());
    }
...
impl Drop for ChunkManager {
    fn drop(&mut self) {
        ...
        for vma_obj in &self.vmas {
            if vma_obj.vma().size() == 0 || vma_obj.vma().perms() == VMPerms::default() {
                continue;
            }
            VMPerms::apply_perms(&vma_obj.vma(), VMPerms::default());
        }
    }
}
