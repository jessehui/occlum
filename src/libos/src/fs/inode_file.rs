...
impl File for INodeFile {
    fn do_read_at(&self, offset: usize, buf: &mut [u8]) -> Result<usize> {
        trace!(
            "[do_read_at]file: {:?}, file offset: {}, buf.len: {}",
        ...
        let page_start_id = offset / PAGE_SIZE; // round floor. TODO: improve this
        let page_end_id = (offset + user_buf_len - 1) / PAGE_SIZE;
    ...
    fn do_write_at(&self, offset: usize, buf: &[u8]) -> Result<usize> {
        trace!(
            "[do_write_at]file: {:?}, file offset: {}, buf.len: {}",
            self.inode.get_inode_num(),
            offset,
            buf.len()
        );
