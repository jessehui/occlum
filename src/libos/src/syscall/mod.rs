...
use core::arch::x86_64::{_fxrstor, _fxsave, _xrstor64, _xsave64};
use std::any::Any;
...
macro_rules! process_syscall_table_with_callback {
    ($callback: ident) => {
        $callback! {
            ...
            (HandleException = 361) => do_handle_exception(info: *mut sgx_exception_info_t, fpregs: *mut FpRegs, xsave_area: *mut u8, context: *mut CpuContext),
...
pub const XSAVE_MASK: u64 = 0xFFFFFFFFFFFFFFFF_u64;
// const AVXREG_SIZE: usize = 2688;

use sgx_trts::enclave::{rsgx_get_thread_data, thread_data_t, SgxThreadData};
