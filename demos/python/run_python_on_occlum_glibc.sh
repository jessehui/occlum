...
if [ ! -d "image/lib/python3.7" ];then
    cp -f $alpine_fs/usr/bin/python3 image/bin
 #   cp -f $alpine_fs/usr/local/lib/libpython3.7m.so.1.0 image/lib
 #   cp -f $alpine_fs/usr/local/lib/libpython3.so image/lib
    cp -f /lib/x86_64-linux-gnu/libpthread.so.0 image/lib
    cp -f /lib/x86_64-linux-gnu/libdl.so.2 image/lib
    cp -f /lib/x86_64-linux-gnu/libutil.so.1 image/lib
    cp -f /lib/x86_64-linux-gnu/libexpat.so.1 image/lib
    cp -f /lib/x86_64-linux-gnu/libz.so.1 image/lib
    cp -f /lib/x86_64-linux-gnu/libm.so.6 image/lib
